...
const UserMessageBar = ({ navigation, userImg, userName, messageTime, messageText, fromMe, seen, read, user }) => {
    ...
    return (
        <TouchableOpacity onPress={onTouchHandle}>
            <View style={styles.container} >
                ...
                <View style={styles.contentContainer}>
                    ...
                    <View style={styles.messageContainer}>
                        <Text numberOfLines={1} style={stylesWithState(read).messageSection}>{senderPrefix + messageText}</Text>
                        <Text>{" " + '\u2022' + " "}</Text>
                        <Text>{messageTime}</Text>
                    </View>
                </View>
                <View style={styles.statusContainer}>
                    {fromMe?(seen?<Avatar2 size={12} source={userImg}/>:<AntDesign name={"checkcircle"} color="grey" />):<></>}
                </View>
            </View>
        </TouchableOpacity>
    )
...
const stylesWithState = (read) => StyleSheet.create({
    nameSection: {
        flex: 1,
